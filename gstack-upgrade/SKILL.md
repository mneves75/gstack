...
[ -z "$_AUTO" ] && _AUTO=$(
  "$HOME/.codex/skills/gstack/bin/gstack-config" get auto_upgrade 2>/dev/null || \
  "$HOME/.agents/skills/gstack/bin/gstack-config" get auto_upgrade 2>/dev/null || \
  "$HOME/.claude/skills/gstack/bin/gstack-config" get auto_upgrade 2>/dev/null || \
  true
)
...
**If "Never ask again":**
```bash
"$INSTALL_DIR/bin/gstack-config" set update_check false
```
Tell user: "Update checks disabled. Run `$INSTALL_DIR/bin/gstack-config set update_check true` to re-enable."
...
for _candidate in \
  ...
  "$_ROOT/.agents/skills/gstack" \
  "$_ROOT/.claude/skills/gstack"; do
...
[ -n "$INSTALL_DIR" ] || { echo "ERROR: gstack not found"; exit 1; }
...
for _candidate in \
  ...
  "$_ROOT/.claude/skills/gstack"; do
  [ -d "$_candidate" ] || continue
  _RESOLVED_LOCAL=$(cd "$_candidate" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_candidate"
    break
  fi
done
...
If `LOCAL_GSTACK` is non-empty, update it by copying from the freshly-upgraded primary install:
```bash
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit that host-local `gstack/` directory when you're ready."
